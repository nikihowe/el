...
os.environ["TOKENIZERS_PARALLELISM"] = "false" # Often helps avoid deadlocks with num_workers > 0
...
model.config.use_cache = False  # Explicitly disable caching for gradient checkpointing again
...
test_input["labels"] = test_input["input_ids"].clone() # For Causal LM, labels are usually shifted input_ids
    ...
    try:
        ...
        if hasattr(outputs, 'loss') and outputs.loss is not None:
            ...
            if hasattr(outputs, 'logits'):
                ...
                active_loss = shift_labels_flat != -100 # Should not be -100 here as we didn't use collator
                if active_loss.any():
                     loss = loss_fct(shift_logits_flat[active_loss], shift_labels_flat[active_loss])
                     print(f"Manually computed loss: {loss.item():.6f}")
                else:
                     print("No active labels for manual loss calculation (all labels might be padding).")
...
def tokenize_function(examples):
    ...
    if len(valid_texts) < len(texts):
        print(f"Warning: Skipped {len(texts) - len(valid_texts)} non-string, empty or None entries in field '{config_values['text_field']}'.")
    ...
    return tokenizer(valid_texts, truncation=True, max_length=config_values['block_size']*2, padding=False) # Added truncation safeguard
...
try:
    raw_dataset = load_dataset('json', data_files=config_values["dataset_path"], split='train') # Explicitly request train split
    raw_datasets = DatasetDict({"train": raw_dataset}) # Put it in a DatasetDict
except Exception as e:
    print(f"Error loading dataset: {e}")
    print("Please ensure the jsonl file is correctly formatted and contains a 'train' structure or adjust split name.")
    sys.exit(1)
...
for i in range(num_examples_to_show):
    ...
    abstract = example.get(config_values['text_field'], "N/A") # Safely get abstract
    print(f"\nExample {i}:")
...
try:
    ...
    print(f"Could not load saved processed datasets: {e}. Processing from scratch...")
    ...
    if config_values['validation_split_percentage'] > 0:
        ...
        print("Validation split percentage is 0. Using the entire dataset for training.")
        split_dataset = DatasetDict({"train": raw_datasets["train"]})
        config_values["eval_strategy"] = "no" # Disable eval if no validation set
    ...
    try:
        if not config_values["overwrite_cache"] and os.path.exists(tokenized_path):
            print(f"Attempting to load tokenized datasets from {tokenized_path}...")
            tokenized_datasets = load_from_disk(tokenized_path)
            # Verify splits match
            if set(tokenized_datasets.keys()) != set(split_dataset.keys()):
                raise ValueError("Cached tokenized dataset splits do not match current splits.")
            print("Successfully loaded cached tokenized datasets.")
        else:
            raise FileNotFoundError("Tokenized cache not found or overwrite requested.")
        ...
        tokenized_datasets = split_dataset.map(
            ...
            remove_columns=next(iter(split_dataset.values())).column_names, # Remove original columns
    ...
    lm_datasets = tokenized_datasets.map(
        group_texts,
        batched=True,
        batch_size=config_values["map_batch_size"], # Process N tokenized entries to form blocks
        num_proc=config_values["preprocessing_num_workers"],
        load_from_cache_file=not config_values["overwrite_cache"],
        desc=f"Grouping texts into chunks of {block_size}",
        writer_batch_size=config_values["map_batch_size"], # Control writing frequency
    ...
    for split in lm_datasets:
        ...
        if initial_count != filtered_count:
            print(f"Filtered {initial_count - filtered_count} empty examples from '{split}' split.")
...
if "train" not in lm_datasets or len(lm_datasets["train"]) == 0:
    print("Error: No training data found after processing. Check dataset and processing steps.")
    sys.exit(1)
...
if "validation" in lm_datasets and len(lm_datasets["validation"]) > 0:
    ...
    if config_values["eval_strategy"] == "no":
        print("Warning: Validation dataset exists but eval_strategy is 'no'. Evaluation will not run.")
...
if config_values.get("warmup_ratio") is not None and config_values["eval_strategy"] != "no":
    ...
    total_train_batch_size = (config_values["per_device_train_batch_size"] *
                              accelerator.num_processes * # Account for distributed training
    ...
    print(f"Calculated warmup steps: {warmup_steps} ({config_values['warmup_ratio']*100:.1f}% of estimated {total_steps} total steps)")
else:
    # Fallback or if ratio isn't used
    warmup_steps = config_values.get("warmup_steps", 0) # Use fixed steps if provided, else 0
...
training_args = TrainingArguments(
    ...
    eval_strategy=config_values["eval_strategy"] if eval_dataset is not None else "no",
    eval_steps=config_values["eval_steps"] if eval_dataset is not None and config_values["eval_strategy"] != "no" else None,
    # Save args
    save_strategy=config_values["eval_strategy"], # Often align save strategy with eval
    save_steps=config_values["save_steps"],
    save_total_limit=config_values["save_total_limit"],
    load_best_model_at_end=True if eval_dataset is not None and config_values["eval_strategy"] != "no" else False, # Load best model based on eval
    metric_for_best_model="loss" if eval_dataset is not None and config_values["eval_strategy"] != "no" else None,
    greater_is_better=False if eval_dataset is not None and config_values["eval_strategy"] != "no" else None, # For loss, lower is better
    # Logging args
...
try:
    train_result = trainer.train() # Potentially resume_from_checkpoint=True/path argument here
    ...
    trainer.log_metrics("train_end", metrics) # Use different key for final train metrics
    ...
    if eval_dataset is not None and training_args.eval_strategy != "no":
        ...
        eval_metrics = trainer.evaluate() # Evaluates the currently loaded model (best or final)
        ...
        try:
            ...
            perplexity = float("inf")
             ...
             perplexity = float("inf")
             print("Warning: 'eval_loss' was None during perplexity calculation.")

        eval_metrics["perplexity"] = perplexity
        trainer.log_metrics("eval_end", eval_metrics) # Use different key for final eval metrics
