...
def count_abstract_tokens(filename, field_name="abstract", approximation_percent=10):
    """
    ...
    print(f"Estimating token count for field '{field_name}' in file: {filename}")
    ...
    try:
        ...
        with open(filename, 'r', encoding='utf-8') as f, \
             tqdm(total=pbar_total, unit=pbar_unit, unit_scale=True,
                  desc=f"Processing first {approximation_percent}%") as pbar:

            for line in f:
                line_bytes = len(line.encode('utf-8'))
                # Check if processing this line would exceed the target
                if bytes_processed + line_bytes > target_bytes and line_count > 0:
                    ...
                    if isinstance(data, dict):
                        ...
                        if abstract_text is not None:
                            if isinstance(abstract_text, str):
                                tokens = tokenizer(abstract_text, add_special_tokens=True)
        ...
        if processed_abstracts_count > 0 and bytes_processed > 0:
            ...
            estimated_total_abstracts = int(processed_abstracts_count * scale_factor)
    ...
    print(f"Estimated total tokens for '{field_name}' in file: ~{estimated_total_tokens:,}")

    # Report errors found in the sample
    if any(errors.values()):
        print("\nEncountered issues in the sample:")
        if errors['json_decode'] > 0:
            print(f"- Lines/Objects with JSON decoding errors: {errors['json_decode']}")
        if errors['missing_field'] > 0:
            print(f"- Objects missing the '{field_name}' field: {errors['missing_field']}")
        if errors['non_string'] > 0:
            print(f"- Objects where '{field_name}' was not a string: {errors['non_string']}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=f'Estimate token count in a specific field of a JSONL file by processing the first N%.')
    parser.add_argument('json_file', help='Path to the large JSON Lines file.')
    parser.add_argument('--field', default='abstract', help='Name of the field containing text to tokenize (default: abstract).')
    parser.add_argument('--percent', type=int, default=10, help='Percentage of file (by size) to process for estimation (default: 10).')
    ...
    try:
        ...
        print("Please install dependencies: pip install transformers tokenizers tqdm")
        sys.exit(1)

    count_abstract_tokens(args.json_file, field_name=args.field, approximation_percent=args.percent)
